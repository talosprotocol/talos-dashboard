/**
 * Talos Dashboard - Audit Stats Proxy
 *
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  try {
    const upstream = await fetch(
      `${GATEWAY_URL}/admin/v1/audit/stats?window_hours=${window_hours}`,
      { method: 'GET', signal: req.signal }
    );

    if (!upstream.ok) {
      return NextResponse.json(
        { code: 'TALOS_ADMIN_FETCH_FAILED', message: 'Failed to fetch audit stats' },
