/**
 * Talos Dashboard - Admin Secrets Proxy
 *
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  try {
    const upstream = await fetch(`${GATEWAY_URL}/admin/v1/secrets`, {
      method: 'GET',
      signal: req.signal,
    });
    ...
    if (!upstream.ok) {
      return NextResponse.json(
        { code: 'TALOS_ADMIN_FETCH_FAILED', message: 'Failed to fetch secrets' },
